...
> **Dividida:** §22.4–§23 (padrão de script, seeds, CI, pentest §22.7–22.8, Makefile) estão em `references/testes-execucao.md`. Numeração contínua entre os dois. A execução de Q.A. mora na skill dedicada **schematize-qa** (`/qa-plan` → `/qa-run`).
...
**Mutation testing (SHOULD)** no domínio em serviços críticos: **Stryker.NET** (`dotnet stryker`), Stryker (JS/TS), `go-mutesting`, `cargo-mutants`.
...
│   ├── unit/                   # delega `dotnet test` (xUnit) por serviço
...
Cobre: auth bypass tentando rotas admin sem cookie, headers de segurança (CSP, X-Content-Type, HSTS), rate limit no login (50 paralelas → expect 429), formato de password hash (**argon2id / PBKDF2** — nunca bcrypt fraco/MD5/SHA1/plaintext), `dotnet list package --vulnerable --include-transitive`, JWT algorithm validation (RS256 only).
...
- **Rota catalog** — JSON gerado a partir do OpenAPI ou inventário de rotas do dispatcher (no Educe: `educe_api_cs/docs/legacy_inventory.json`).
...
**Setup**: `bash tests/simulated/setup.sh` cria as 3 personas no DB com hash **argon2id / PBKDF2**.
...
- **.NET (primário): `dotnet test` (xUnit primário / NUnit) com coverlet (`--collect:"XPlat Code Coverage"`) + FsCheck + Stryker.NET**
...
- **Determinismo e thread-safety sempre** (.NET): `dotnet test` reprodutível + testes de concorrência/detector de corrida sobre estado compartilhado (`lock`/`Interlocked`/`Channel`). Teste flaky por corrida = bug, não "re-roda".
- **Caminho de erro é obrigatório, não opcional.** Para cada função, testar o sucesso **e** as falhas: input inválido, dependência que retorna erro, timeout, nil/null, coleção vazia, divisão por zero, overflow. Cobertura de 80% só de happy-path é cobertura mentirosa.
...
- **Property-based testing (SHOULD → MUST em domínio crítico):** **FsCheck** (.NET), `fast-check` (TS), `gopter`/`rapid` (Go), `proptest` (Rust). Idempotência, round-trip (encode→decode), invariantes de agregado. Encontra o edge case que você não imaginou.
- **Mutation testing no domínio crítico** (§22, **Stryker.NET**/`go-mutesting`/`cargo-mutants`): se o teste não pega a mutação, o teste é decorativo. **Mutation score mínimo definido por serviço crítico**, não só line coverage.
- **Boundary obrigatório:** `0`, `-1`, `1`, `MAX`, `MAX+1`, vazio, um, muitos. O bug mora na borda.
- **Ferramental .NET padrão:** asserções via **FluentAssertions** (mensagem de falha legível), mocks/fakes **só nas bordas** via **Moq** ou **NSubstitute** (nunca mockar o próprio domínio sob teste), integração com dependências reais (DB, broker) via **Testcontainers (.NET)** — container efêmero por run, não mock de banco.
- **Proibido teste que não pode falhar:** assert ausente, `Assert.True(true)` / `true.Should().BeTrue()`, mock que devolve o próprio input esperado. Revisão de PR rejeita teste tautológico.
