...
- **CPU-bound de verdade** → `await Task.Run(...)` (offload deliberado), e mesmo
  assim medido; I/O nunca vai em `Task.Run` (não é isso que ele resolve).
...
**VETADO**
- Assinar método async sem `CancellationToken` "porque ninguém cancela" — o
  analyzer `CA2016` (propagar token) trava CI.
...
- **`JoinSet` do .NET é `Task.WhenAll` observado:** toda task de fundo tem seu
  resultado/exceção **observado** — task que falha reporta (log estruturado +
  métrica) e decide retry/parar; não morre em silêncio (piso "erro nunca engolido").
