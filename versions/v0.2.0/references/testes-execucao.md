...
> Parte da skill **schematize-csharp**. Continuação de `testes.md` (numeração **preservada**, §22.4+): padrão de script de teste, seeds/personas, integração com CI, pentest (§22.7–22.8) e o Makefile padrão (§23). A disciplina de execução de Q.A. mora na skill dedicada **schematize-qa**.
...
- **Todo e-mail de persona/seed/fixture usa o DOMÍNIO DE TESTE em rota nula** — `<papel>+<run-id>-<n>@test.<domain>` (null MX + SPF `-all` + DMARC `p=reject`) ou TLD reservado (`.test`/`.invalid`/`.example`). **VETADO** `@gmail.com`/`@hotmail.com`, domínio de cliente/terceiro, e-mail de pessoa real (**inclusive o seu**) e o domínio de **produção** — o auth manda **Email OTP always-on** a cada conta criada, e um laço de seed/`Bogus` vira disparo em massa. Piso e motivo em `references/iam.md` (§3.1) e `schematize-engineering/references/efeitos-externos.md`; gate no CI: `grep -rE "gmail|hotmail|outlook|yahoo|icloud" tests/ seeds/` **trava**.
- **Teste que vê o vermelho do guard:** o suite tenta entregar para `@gmail.com` em ambiente não-prd e **espera a exceção** (`await Assert.ThrowsAsync<ExternalRecipientBlockedException>(...)`, provando que o inner **não** recebeu nada), e prova o **abort do cap por execução** (`MailCapExceededException`). Sem esse teste, o guard é intenção.
...
A execução de Q.A. da casa (plan-first, aprovação obrigatória, faseado/assistido ou autônomo com watchdog) mora na skill dedicada **schematize-qa** (`/qa-plan` → `/qa-run`): planeja tudo, gera o MD de passo a passo no `<projeto>_archive/qa/`, pede aprovação antes de rodar e trava nos gates.

No recorte **C#/.NET**, o wrapper `/cs-qa` aplica essa disciplina com as ferramentas de teste da plataforma — `dotnet test` (xUnit), integração via `WebApplicationFactory`/`Testcontainers` e o pentest da schematize-pentest. Nada de Q.A. roda sem plano aprovado registrado.
