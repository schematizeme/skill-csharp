...
inegociável. Este arquivo é o recorte **backend/C#** do modelo agnóstico
(`schematize-engineering/references/iam.md`): mantém **todas as regras normativas** e
as concretiza no serviço de auth como **microserviço C#/.NET (ASP.NET Core)**
(`<projeto>_auth_cs`).
...
- **Senha por padrão, opcional por escolha:** o usuário **cria senha no cadastro** (padrão
  cultural; **argon2id** via **Konscious.Security.Cryptography** — ou PBKDF2 via
  `Rfc2898DeriveBytes` — + verificação contra base de vazadas/HIBP k-anonymity), mas o
    ...
    ILogger<GuardedEmailProvider> logger) : IEmailProvider
{
    ...
    private void EnsureDeliverable(string to)
    {
        ...
        logger.LogError("mail BLOQUEADO: {Recipient} em env={Environment}", to, _options.EnvironmentName);
...
personas em `references/testes-execucao.md` (§22.5).
...
- Toda mudança **notifica todos os canais verificados**; remover o **último fator forte** =
  **ação com atraso cancelável** (janela pra abortar se for ataque) — implementada como job
  agendado no auth, cancelável até disparar.
...
- **Motor de relação (ReBAC), ex. OpenFGA/SpiceDB** — hand-rolar authz é onde vazam
  privilégios. O serviço C# **não implementa o motor**: fala com o OpenFGA/SpiceDB via
  client gRPC/HTTP. Autorização em **tuplas** `(objeto, relação, usuário/userset)`:
...
- **Anti-automação sempre:** rate-limit + **backoff exponencial** e **lockout progressivo por
  conta+IP**; OTP curto, single-use, `jti` na denylist. Barra o abuso sem derrubar o serviço.
...
- **F0** Núcleo de identidade (ID imutável, N identificadores, verificação, Resend/Twilio
  plugáveis por interface C# via DI, email OTP always-on) — já como **microserviço C#/.NET separado**
  em `auth.<domain>`.
