---
description: schematize-csharp — força/audita/scaffolda o IAM da casa (identidade≠email, ≥2 fatores, ReBAC multi-tenant, sessão longa/logout irreversível) como microserviço C#/.NET separado em auth.<domain>, ou atualiza um auth existente
...
**backend/C#/.NET (ASP.NET Core)**. Plan-first: **audita, mostra o plano, pede aprovação, então executa.** Use
...
- `bootstrap` — scaffolda o IAM do zero como **microserviço C#/.NET (ASP.NET Core) separado**.
...
- Microserviço **C#/.NET (ASP.NET Core)** próprio `<projeto>_auth_cs` + front próprio `<projeto>_authfront`,
  servidos em **`auth.<domain>`** — **VETADO** monolith apensado (nem uma pasta `Auth/` do
  serviço principal). **ASP.NET Core Identity ≠ IAM da casa** — pode servir de peça interna, mas
  não substitui a app de auth separada, a identidade≠email, os ≥2 fatores nem o ReBAC.
- Repo/deploy/**user Linux + systemd isolados** por conta própria (casa com `ops.md` da base (`schematize-engineering`) §3).
- App principal e clientes **delegam por OIDC/OAuth2.1 + PKCE**; a **chave de assinatura só
  no auth C#/.NET**, consumidores validam por **JWKS público** (middleware ASP.NET Core cacheando o JWKS).
...
## 3. Fatores (≥2 sempre)
- **Passkey/WebAuthn no núcleo** (lib madura, ex. `Fido2` .NET); TOTP (`Otp.NET`)/push;
  **email OTP (Resend) always-on inclusive HML** (só operador desliga); **Twilio** p/
  telefone; providers **plugáveis como interfaces C# via DI** (`IEmailProvider`/`ISmsProvider`/
  `IPushProvider`, adaptadores na borda, registrados no container).
- **Senha por padrão** (**argon2id**, ex. `Isopoh.Cryptography.Argon2` — ou PBKDF2 do
  `Microsoft.AspNetCore.Cryptography.KeyDerivation` como fallback — + HIBP), **opcional no seletor**.
- **Senha + Email OTP JÁ é o 2FA baseline** (acesso pleno desde o cadastro). Fator forte (passkey/TOTP) é **nudge + step-up just-in-time** (op sensível / risco), **NUNCA muro pré-login** — barrar o acesso por falta de fator forte é o círculo infinito / bug de bootstrap **VETADO** pela norma (`iam.md` §3/§4).
  middleware/policy barra o token de bootstrap fora do enrolamento).
...
- **Identidade global, papéis por tenant** (membership). Motor **ReBAC (OpenFGA/SpiceDB)** —
  o auth C#/.NET **fala com o motor via client**, não implementa authz na mão; tuplas
  `(objeto, relação, usuário)`; **RBAC granular** (`recurso:ação`, papéis customizados) +
  **ABAC** (conditional tuples). **PDP = Check API / PEP = middleware + policy handler ASP.NET Core,
  deny-default, enforcement server-side, token fino, decisão auditada.** Escrita de tupla via outbox
...
- **Sessão 7 dias por padrão; pergunta se confiável → 90 dias** (access token curto com
  ...
  sensível. Refresh opaco (`RandomNumberGenerator`), reuso → revoga a **família**.
...
migrado nasce microserviço C#/.NET separado. (Legado Node casa com `schematize-node`.)
...
legado→novo e a ordem de corte. Confirme: auth é microserviço C#/.NET à parte? identidade≠email?
