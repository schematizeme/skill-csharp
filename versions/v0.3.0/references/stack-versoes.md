...
| **CQRS (opcional)** | **nenhuma lib por default** — um `ICommandHandler<T>`/`IQueryHandler<T>` seu, registrado no DI, resolve o caso comum em ~30 linhas | ✔ **2026-08-21: MediatR virou COMERCIAL (licença paga) em 02/07/2025.** Prescrevê-lo como default contradizia `references/seguranca.md`, que **bloqueia dependência proprietária sem ADR** — a skill se contradizia. Se o projeto quiser MediatR (ou outra lib de mediator), é **ADR de exceção** com o custo de licença registrado, não default |
| **RPC** | **gRPC** (`Grpc.AspNetCore`) | contrato entre serviços |
| **Resiliência** | **`Microsoft.Extensions.Http.Resilience`** (`AddStandardResilienceHandler`) | é o **caminho oficial do Polly v8** no .NET: pipeline padrão (rate limiter + total timeout + retry + circuit breaker + attempt timeout) plugado no `IHttpClientFactory` com uma linha, e com telemetria. Usar o Polly cru continua válido para pipeline não-HTTP |
