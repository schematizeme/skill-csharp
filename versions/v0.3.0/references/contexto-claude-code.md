...
> Parte da skill **schematize-csharp**. **É o DONO do handoff de contexto**: o §34.1 do `entrega.md` **da base** (`schematize-engineering`) é o *gatilho* (quando parar e arquivar); o *como* — formato dos dois MDs, hooks, `/compact`, o que entra em cada seção — é aqui, e só aqui. Implementa também a §28 (archive). Conteúdo específico de ferramenta (Claude Code / VS Code) — o princípio normativo está no `entrega.md` **da base** (`schematize-engineering` -> `references/entrega.md` §34.1) — o recorte local deixou de repeti-lo quando virou ponteiro.
...
## Comando `/cs-cc` (context compact) — handoff manual sob demanda

No Claude Code, comandos e skills são unificados: ambos criam um `/nome`. Há dois formatos — `.claude/commands/<nome>.md` (legado, mais simples) e `.claude/skills/<nome>/SKILL.md` (recomendado; suporta `/nome` mais invocação autônoma, e aceita o campo `command:` no frontmatter). Uma pasta de skill = um comando, então um `/cs-cc` próprio é um arquivo de comando dedicado.

Bundlado em `assets/commands/cs-cc.md`. Instalação:

1. Copie `assets/commands/cs-cc.md` para `.claude/commands/cs-cc.md` na raiz do projeto.
2. Digite `/cs-cc` na sessão (ou `/cs-cc <foco>` pra direcionar o resumo, via `$ARGUMENTS`).

O `/cs-cc` gera dois MDs em `<projeto>_archive/context/`:
...
E então roda `/compact` com foco. Diferença pro automático: o StatusLine/PreCompact é o gatilho **automático** no limite; o `/cs-cc` é o handoff **manual** que você dispara quando quiser fechar um marco. Os dois gravam no mesmo `<projeto>_archive/context/`.

Alternativa em formato skill (auto-invocável além do `/cs-cc`): criar `.claude/skills/cs-cc/SKILL.md` com frontmatter `name`, `description` e `command: /cs-cc`, e o mesmo corpo do `cc.md`.
