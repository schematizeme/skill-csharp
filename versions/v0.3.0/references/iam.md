...
> **PONTEIRO, não cópia.** A normativa agnóstica de IAM é da base: **`schematize-engineering`** →
> `references/iam.md` — inclusive as regras que decidem se o motor funciona (§5.1: **outbox
> transacional** na escrita de tupla ReBAC, **PDP fail-closed**, **denylist de `jti` como consulta
> obrigatória**), o **refresh opaco CSPRNG hasheado** (§6), a **comparação em tempo constante** na
> negação deceptiva (§9), o **CSPRNG obrigatório** em toda aleatoriedade de segurança e os
> **parâmetros mínimos de argon2id** (§2.1). Leia lá primeiro.
>
> **Onde este arquivo divergir da base, a BASE MANDA.** Aqui fica o recorte idiomático: a topologia
> do serviço, as bibliotecas, e o guard de efeito externo da §3.1 — que é código compilado e
> testado, não prosa.
...
- **Microserviço de auth em C#/.NET (ASP.NET Core)** (`<projeto>_auth_cs`) + **front de auth**
  ...
  por conta própria (casa com o isolamento por app do `ops.md` da base (`schematize-engineering`) §3). Comprometer o app
...
- **Senha por padrão, opcional por escolha:** o usuário **cria senha no cadastro** (padrão
  ...
  **seletor de modos de autenticação permite marcá-la como opcional** e viver de passkey/OTP/app.
  ...
  > Quando for mexer no legado, use a **API estática** `Rfc2898DeriveBytes.Pbkdf2(...)` — o
  > **construtor** é obsoleto (**`SYSLIB0041`**) e, com o `TreatWarningsAsErrors` que esta skill
  > exige, o código que segue o conselho antigo **nem compila**.
