 ...
 *
 * SCAFFOLD: cobre C#, TS/JS e Go de forma pragmática (regex). Em C# reconhece o
 * doc-comment XML `///` (com `O quê:`/`Onde:`/`Efeitos:` dentro de `<summary>` ou
 * como linhas soltas) acima de método/classe/record/interface. Para AST confiável
 * em C#, prefira Roslyn (`Microsoft.CodeAnalysis`)/ctags. Estenda `declRe` para a
 * sua stack. A ideia é que o índice seja DERIVADO do código comentado, não mantido
 * à mão — CI compara gerado vs commitado.
 */
import fs from "node:fs";
import path from "node:path";

const ROOT = process.argv[2] ?? "src";
const EXTS = new Set([".cs", ".ts", ".tsx", ".js", ".mjs", ".go", ".rs"]);

// Detecta uma declaração de função/tipo logo após o doc-comment (best-effort).
// C#: método (modificadores + tipo de retorno + Nome() ) e class/record/struct/interface.
const declRe = /(?:export\s+)?(?:async\s+)?function\s+(\w+)|(?:func\s+(?:\([^)]*\)\s+)?(\w+))|(?:(?:export\s+)?const\s+(\w+)\s*=\s*(?:async\s*)?\()|(?:(?:public|private|protected|internal|static|sealed|virtual|override|async|partial|abstract)\s+)+[\w<>\[\],\.\?]+\s+(\w+)\s*(?:<[^>]*>)?\s*\(|(?:public|private|protected|internal|sealed|abstract|static|partial)\s+(?:class|record|struct|interface|enum)\s+(\w+)/;
...
for (const file of walk(ROOT)) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!name) continue;
    // Ignora palavras-chave capturadas por engano como nome (guarda C#).
    if (/^(if|for|foreach|while|switch|using|return|lock|catch|else)$/.test(name)) continue;
