...
Atualize o **índice de funcionalidades** (§39) — **fonte da verdade** do projeto e base do MAPA (§4). O índice é **exaustivo**: **uma entrada por função** (método/membro), de **cada** serviço, **com grafo**. Ele **enumera** o sistema; não resume.

## 1. Enumere e CONTE (a verdade do que existe)

Antes de gerar, descubra **todas** as funções do alvo `${ARGUMENTS:-src}` — públicas e privadas, incluindo métodos, handlers, jobs, `local functions` e lambdas nomeadas. Prefira **AST confiável**: **Roslyn** (analyzers / `Microsoft.CodeAnalysis`, ex. um analyzer ou script `csharp` que percorre a árvore sintática) ou **ctags** dão a contagem exata de declarações e distinguem método de chamada — use-os **sempre que disponíveis**. Só caia no ripgrep como aproximação quando não houver AST:

- **C# (primário):** métodos → `rg -n '(public|private|protected|internal|static|async|sealed)\s+.*\b\w+\s*\('` ; tipos → `rg -n '\b(class|record|struct|interface)\s+\w+'`.
- **Go:** `rg -n '^\s*func '`
...
Fonte: `scripts/build-index.mjs` se existir; senão, extraia dos **XML doc-comments (`///`)** dos membros públicos (§3). **Nenhuma** função fica de fora por "não ser relevante".
...
- Se `scripts/build-index.mjs` sair com código 1, há membro público sem XML doc (`///`) (§3): corrija **na origem** (o quê + de onde→pra onde), não no índice.
